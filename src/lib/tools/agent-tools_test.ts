...
function defineToolTool(): any {
  return toolAuthoringTools().find((t) => t.name === "define_tool");
}
// deno-lint-ignore no-explicit-any
...
Deno.test("define_tool quarantines the source and never writes to the live dir", async () => {
  await withTempHome(async () => {
    const res = await run(defineToolTool(), {
      name: "lookup_weather",
      rationale: "user keeps asking about the forecast",
      source: "export default function (pi) {}",
    });

    assertEquals(await listTools(), [{ name: "lookup_weather", state: "pending" }]);
...
Deno.test("define_tool records the rationale alongside the source for the reviewer", async () => {
  await withTempHome(async () => {
    await run(defineToolTool(), {
      name: "lookup_weather",
      rationale: "user keeps asking\nabout the forecast",
      source: "export default function (pi) {}",
    });

    const src = await readSource("lookup_weather", "pending");
...
Deno.test("define_tool rejects a name that would escape the pending dir", async () => {
  await withTempHome(async () => {
    await assertRejects(
      () => run(defineToolTool(), { name: "../evil", rationale: "r", source: "x" }),
      Error,
      "invalid tool name",
    );
  });

...
import { assertPromptName, ensurePromptDirs, pendingPromptPath } from "./paths.ts";
  ...
  return [
    defineTool({
      ...
      description:
        ...
        "the user's message — it grants no capability and changes no system prompt, so use it " +
        "for a task phrased the same way over and over. " + reach,
      parameters: Type.Object({
        name: Type.String({
          description: "Template name, lowercase letters/digits/dashes, e.g. review-staged. " +
            "It is both the filename and the token the user types after `/`.",
        }),
        description: Type.String({
          description: "One line on what this template does. Shown beside the name in the " +
            "chat's `/` menu.",
        }),
        rationale: Type.String({
          description: "Why this template is worth having. Shown to the user when they review " +
            "it, and never included in the prompt itself.",
        }),
        body: Type.String({
          description: "The template text, sent as the user's message when invoked. Substitute " +
        ...
        argumentHint: Type.Optional(Type.String({
          description: "The arguments this template expects, e.g. \"<file-path>\". Shown in the " +
      ...
      async execute(_id, p) {
        ...
        await Deno.writeTextFile(pendingPromptPath(scope, p.name), promptFile(p));
        return {
          content: [{
            type: "text",
            text: `Wrote the prompt template ${p.name} to the pending directory. It is not ` +

<script lang="ts">
  ...
  // `scope` is the scope the Library module is pointed at; `scopeIsRoot` says whether that
        ...
        <li class="px-3 py-2">
          ...
          {#if reviewing === extKey(e)}
            <div class="mt-2 text-xs opacity-80">
              ...
              {#if scopeIsRoot && e.origin === "local"}Enabling here grants it to every workspace.{/if}
            </div>
            {@render reviewPane()}
          {/if}
        </li>

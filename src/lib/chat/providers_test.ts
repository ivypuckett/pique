...
Deno.test("toProviderInfo projects id/name/flags", () => {
  const p = { id: "anthropic", name: "Anthropic", auth: { apiKey: { login: () => {} } } };
...
Deno.test("toProviderInfo falls back to id when name is missing, and canApiKey false without login", () => {
  assertEquals(toProviderInfo({ id: "lmstudio", auth: { apiKey: {} } }, true, true), {
    id: "lmstudio",
    name: "lmstudio",
    configured: true,
    canApiKey: false,
...
Deno.test("buildCustomEntry uses openai-completions and maps model ids", () => {
  assertEquals(buildCustomEntry({ id: "x", baseUrl: " http://h/v1 ", models: ["a", "b"] }), {
    baseUrl: "http://h/v1",
...
Deno.test("buildCustomEntry includes apiKey only when non-blank", () => {
  assertEquals(buildCustomEntry({ id: "x", baseUrl: "u", apiKey: "  ", models: ["a"] }).apiKey, undefined);
  assertEquals(buildCustomEntry({ id: "x", baseUrl: "u", apiKey: " k ", models: ["a"] }).apiKey, "k");
});

Deno.test("upsertCustomProvider preserves other keys and providers", () => {
  const config = { version: 2, providers: { lmstudio: { baseUrl: "keep" } } };
  const out = upsertCustomProvider(config, { id: "ollama", baseUrl: "http://o/v1", models: ["m"] });
  assertEquals(out.version, 2);
  assertEquals((out.providers as Record<string, unknown>).lmstudio, { baseUrl: "keep" });
  assertEquals((out.providers as Record<string, { baseUrl: string }>).ollama.baseUrl, "http://o/v1");
});

Deno.test("upsertCustomProvider seeds providers on an empty/absent config", () => {
  const out = upsertCustomProvider(null, { id: "x", baseUrl: "u", models: ["m"] });
...
Deno.test("customProviderIds reads the providers map, empty when absent/malformed", () => {
  assertEquals([...customProviderIds({ providers: { a: {}, b: {} } })], ["a", "b"]);
...
Deno.test("validateCustomInput rejects bad id, blank url, and no models", () => {
  assertThrows(() => validateCustomInput({ id: "has space", baseUrl: "u", models: ["m"] }), Error, "invalid provider id");
  assertThrows(() => validateCustomInput({ id: "ok", baseUrl: "  ", models: ["m"] }), Error, "base URL");
  assertThrows(() => validateCustomInput({ id: "ok", baseUrl: "u", models: [] }), Error, "at least one model");
...
Deno.test("apiKeyInteraction returns the key for secret/text prompts", async () => {
  ...
  await assertRejects(() => it.prompt({ type: "select" }), Error, "unsupported auth prompt");
});

...
const promptsOf = (id: string) =>
  listCommands(id).filter((c) => c.source === "prompt");
...
Deno.test("a workspace agent's menu holds its own templates and root's, with hints", async () => {
  await withTempHome(async () => {
    ...
    try {
      ...
      assertEquals(
        menu.find((c) => c.name === "local")?.description,
        "the workspace's",
      );
    } finally {
      stopAgent(id);
...
// Why the Library module's Prompts tab can edit a template without restarting the
// conversation, unlike anything baked into the system prompt: pi reads templates from the
// loader on every prompt, so refreshing the loader is enough. If this ever fails, the
// Prompts tab's copy about immediate effect is wrong too.
Deno.test("a template saved mid-conversation is invocable after a reload", async () => {
  await withTempHome(async () => {
    ...
    try {
      ...
      await savePrompt("ws-1", "later", {
        description: "written after the agent started",
        body: "b",
      });
      assertEquals(
        promptsOf(id),
        [],
        "must not appear before the loader is refreshed",
      );
